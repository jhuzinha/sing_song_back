import { deleteData, disconnectPrisma } from "./Factory/scenarioFactory";
...
import {
  ...
  randomAmount,
} from "../utils Test/recommendationRandom";
import {
  verifyExitsRecommendation,
  createRecommendation,
  insertMany,
} from "./Factory/recommendationFactory";
...
describe("ROUTE RECOMMENDATIONS", () => {
  ...
  it("POST /recommendations/:id/upvote", async () => {
    ...
    const result = await supertest(app).post(
      `/recommendations/${existRecommendation.id}/upvote`
    );
    const updateRecoomendation = await verifyExitsRecommendation(
      existRecommendation.name
    );
  ...
  it("POST /recommendations/:id/downvote", async () => {
    ...
    const existRecommendation = await createRecommendation(recommendation);
    const result = await supertest(app).post(
      `/recommendations/${existRecommendation.id}/downvote`
    );
    const updateRecoomendation = await verifyExitsRecommendation(
      existRecommendation.name
    );
  ...
  it("GET /recommendations/:id", async () => {
    ...
    const result = await supertest(app).get(
      `/recommendations/${existRecommendation.id}`
    );
    expect(result.body).not.toBeFalsy();
    expect(result.body).toBeInstanceOf(Object);
